...
function i(a) {
    ...
    var d = c.enter().append("svg:g").attr("width", 100);
    d
        .append("svg:polygon")
        .attr("points", h)
...
function k(a) {
    var c = ["#49acc0", "#FFC65D", "#5f9b7e", "#93648D", "#ffcccc", "#66ffcc", "#cc33ff", "cc6699"],
        d = [-.1, -.05, 0];
    if (1 == a.depth) {
...
var chart = function (d3) {
        ...
        tickLabels = ["Certified", "Certified Withdrawn", "Withdrawn", "Denied"];
        var chart = {},
            rect = {
                top: 20,
                right: 20,
    ...
    width = 900,
    height = 600,
    rad = Math.max(width, height) / Math.PI - 25,
    q = k,
    r = {
        w: 220,
        h: 40,
...
path.append("svg:text")
    ...
    .attr("dx", "15").attr("dy", ".1em").text(function (a) {
        if (a.value < 10)
            return "";
    if (a.key.split(' ').length > 1)
        return a.key.match(/[A-Z]/g).join('');
    return a.key
})
    .attr("display", function (a) {
        return a.children ? null : "none"
